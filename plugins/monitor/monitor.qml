...
Item {
  ...
  function openDetail(pid) {
    ...
    root.run(["plugin-exec", pluginKey, "detail", String(pid)], function(out) {
      try {
        ...
        actionChoice = 0; resultMessage = ""; monitor.forceActiveFocus()
  ...
  function openConfirmation(pid, choice) {
    if (pid < 0) return
    if (!detailVisible || detailData.pid !== pid) {
      detailData = { pid: pid, name: processes[selectedIndex] ? processes[selectedIndex].name : "process" }
    }
    confirmVisible = true; actionChoice = choice === 2 ? 2 : 1; resultMessage = ""; monitor.forceActiveFocus()
  }

  function cancelOverlay() {
    if (confirmVisible) { confirmVisible = false; actionChoice = 0; monitor.forceActiveFocus(); return true }
    if (detailVisible) { detailVisible = false; resultMessage = ""; monitor.forceActiveFocus(); return true }
    if (filterField.activeFocus) { filterField.focus = false; monitor.forceActiveFocus(); return true }
    return false
  }

  function signalProcess(signal) {
    var pid = detailData.pid || selectedPid
    root.run(["plugin-exec", pluginKey, "kill", String(pid), signal], function(out) {
      try {
  ...
  Column {
      ...
      Repeater {
        ...
        Rectangle {
          ...
          Row {
            ...
            Column {
              Text { text: modelData.label; color: Qt.darker(Color.foreground, 1.5); font.pixelSize: Style.font.caption }
              Text { text: modelData.value; color: Color.foreground; font.pixelSize: Style.font.bodySmall; font.bold: true }
            }
          }
        }
      }
    ...
    Row {
      ...
      Rectangle {
        ...
        Text { x: Style.space(8); y: Style.space(5); text: "CPU"; color: Color.accent; font.pixelSize: Style.font.caption; font.bold: true }
        Canvas {
      ...
      Rectangle {
        width: (parent.width - Style.space(8)) / 2; height: Style.space(76); radius: Style.cornerRadius
        color: Color.menu.selectedBackground; border.color: Color.popups.border; border.width: 1
        Text { x: Style.space(8); y: Style.space(5); text: "RAM " + formatMemory(memoryUsedKb) + " / " + formatMemory(memoryTotalKb); color: Color.accent; font.pixelSize: Style.font.caption; font.bold: true }
        Canvas {
    ...
    Row {
      ...
      Repeater {
        ...
        Rectangle {
          ...
          Text { anchors.centerIn: parent; text: modelData.label + (sortKey === modelData.key ? (sortDescending ? " ▼" : " ▲") : ""); color: sortKey === modelData.key ? Color.accent : Qt.darker(Color.foreground, 1.5); font.pixelSize: Style.font.caption; font.bold: true }
    ...
    ListView {
      id: processList
      width: parent.width; height: Style.space(245); clip: true
      ...
      delegate: Rectangle {
        ...
        Row {
          ...
          Text { width: parent.width * 0.47; anchors.verticalCenter: parent.verticalCenter; text: modelData.name; color: Color.foreground; font.pixelSize: Style.font.caption; elide: Text.ElideRight }
          Text { width: parent.width * 0.19; anchors.verticalCenter: parent.verticalCenter; text: Number(modelData.cpu).toFixed(1); color: modelData.cpu > 50 ? Color.accent : Color.foreground; font.pixelSize: Style.font.caption; horizontalAlignment: Text.AlignHCenter }
          Text { width: parent.width * 0.20; anchors.verticalCenter: parent.verticalCenter; text: Number(modelData.ram).toFixed(1); color: Color.foreground; font.pixelSize: Style.font.caption; horizontalAlignment: Text.AlignHCenter }
        }
        MouseArea {
      ...
      Text { anchors.centerIn: parent; visible: processes.length === 0; text: loading ? "Sampling…" : "No matching processes"; color: Qt.darker(Color.foreground, 1.5); font.pixelSize: Style.font.bodySmall }
    }
  }

  Rectangle {
    id: overlay
    anchors.fill: parent; z: 20; visible: detailVisible || confirmVisible
    ...
    Column {
      ...
      Text { text: "State  " + (detailData.state || "—") + "     Memory  " + formatMemory(detailData.rssKb || 0) + "     Threads  " + (detailData.threads || "—") + "     UID  " + (detailData.uid === undefined ? "—" : detailData.uid); color: Qt.darker(Color.foreground, 1.5); font.pixelSize: Style.font.bodySmall }
      Row {
      ...
      Text { visible: resultMessage !== ""; text: resultMessage; color: Color.accent; font.pixelSize: Style.font.bodySmall; anchors.horizontalCenter: parent.horizontalCenter }
    }

    Rectangle {
      anchors.fill: parent; anchors.margins: Style.space(36); z: 2; visible: confirmVisible
      ...
      Column {
        ...
        Text { anchors.horizontalCenter: parent.horizontalCenter; text: "Signal " + (detailData.name || "process") + " (PID " + detailData.pid + ")?"; color: Color.foreground; font.pixelSize: Style.font.body; font.bold: true }
        Text { anchors.horizontalCenter: parent.horizontalCenter; text: actionChoice === 2 ? "Force kill cannot be handled or cleaned up." : "Terminate lets the process shut down cleanly."; color: Qt.darker(Color.foreground, 1.5); font.pixelSize: Style.font.caption }
        Row {
          anchors.horizontalCenter: parent.horizontalCenter; spacing: Style.space(8)
          Repeater {
            ...
            Rectangle {
              ...
              color: actionChoice === index ? Color.menu.selectedBackground : "transparent"
              border.color: actionChoice === index ? Color.accent : Color.popups.border; border.width: 1
              Text { anchors.centerIn: parent; text: modelData; color: actionChoice === index ? Color.accent : Color.foreground; font.pixelSize: Style.font.caption; font.bold: actionChoice === index }
